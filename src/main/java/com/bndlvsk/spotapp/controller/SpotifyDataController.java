...
public class SpotifyDataController {
    ...
    public ResponseEntity<TracksDTO> tracks(@RequestParam String range,
                                            @AuthenticationPrincipal OAuth2UserWrapper principal) {
    ...
    public ResponseEntity<ArtistsDTO> artists(@RequestParam String range,
                                              @AuthenticationPrincipal OAuth2UserWrapper principal) {
        long userId = principal.getUserId();
    ...
    public ResponseEntity<GenresDTO> genres(@RequestParam String range,
                                            @AuthenticationPrincipal OAuth2UserWrapper principal) {
        long userId = principal.getUserId();
        ArtistsDTO artistsDTO = spotifyApiService.getTopArtists(userId, range);
    ...
    public ResponseEntity<PlaylistDTO> createPlaylist(@RequestParam String range,
                                                      @AuthenticationPrincipal OAuth2UserWrapper principal) {
        long userId = principal.getUserId();
    ...
    public ResponseEntity<AnalysisDTO> playlistAnalysis(@AuthenticationPrincipal OAuth2UserWrapper principal,
                                                        @Valid @RequestBody PlaylistInfo playlistInfo) {
    ...
    public ResponseEntity<Map<String, Double>> matchUsers(@AuthenticationPrincipal OAuth2UserWrapper principal,
                                                          @RequestParam("user_id") long matchUserId) {
        long userId = principal.getUserId();
